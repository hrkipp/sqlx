This is a fork of the very useful `github.com/jmoiron/sqlx` package, which makes a small change to the db struct. Specifically, it changes the embeded `*sql.DB` struct into an interface which mirrors the struct. This allows the 
library to be composed with other sql libraries which attempt to mirror the `sql.DB` struct.
